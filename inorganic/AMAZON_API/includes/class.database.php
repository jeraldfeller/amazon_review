 ...
 * @version   1.4.6 gomco - both update and insert escaped, on duplicate added, insert unknown column added, close added
 **/
...
class Database
{
	...
	private $_result;
	var $error = '';
	var $unknownColumn = false;
	var $debugUnknownColumn = true;
	var $debug = false;	
	...
	public function insert($table, $data)
	{
		...
		$this -> execute();
		if($this -> unknownColumn) return -1;
		else return $this -> _mysqli -> insert_id;
	}
	...
	/**
	 * Inserts data into table on duplicate key Update.
	 *
	 * @param string $table Name of the table
	 * @param array $data The array which contains the coulumn name and values to be
	 * inserted.
	...
	public function insertUpdateOnDuplicate($table, $data)
	{
		if (isset($this -> table_prefix))
			$table = $this -> table_prefix . $table;
		...
		$this -> _query = "INSERT INTO " . $table . " (" . implode(', ', $keys) . ") VALUES (" . implode(', ', $values) . ") ON DUPLICATE KEY UPDATE " . implode(', ', $valstr).";";
		$this -> execute();
		return $this -> _mysqli -> insert_id;
	}
	...
	{
		if ($this -> debugUnknownColumn OR $this -> debug) $this -> error = mysqli_error($this -> _mysqli);
		if ($this -> debugUnknownColumn){
			if(stripos($this -> error,"Unknown column") !== false) $this -> unknownColumn = true;
		}
